...
8. **Phase 4** — fill module docs: **non-leaves first** (establish module-wide invariants), **then leaves** (reference parent invariants by ID). Every leaf goes through a 4-pass lifecycle: **4.1 writer + 4.2 reviewer dispatched as a parallel pair** for each leaf — writer drafts `modules/M####-<slug>.md` (DRAFT sentinel), reviewer independently re-derives structural fields from source and writes `.meta/cross-checks/M####-<slug>.review.json` (with explicit instruction NOT to read the writer's draft) → **4.3 reconcile** (orchestrator diffs per leaf, marks agreed rows `confidence: high`, re-verifies writer-only rows, adds genuine reviewer-only rows) → **4.4 Quality Gate** (14 mechanical pass/fail checks; fails route back to writer for targeted patch, NOT full rewrite; 3-round escalation to confidence: low + R####). DRAFT sentinel flips to END-OF-MODULE only after 4.4 passes. **Multiple leaves run in parallel** (up to 8 pairs = 16 simultaneous subagents per concurrency budget). Non-leaves skip 4.2/4.3 but still pass through 4.4.
9. **Phase 5** — stitch global interaction maps under `interactions/`.
10. **Phase 6/7** — audit coverage in two passes: **6.1 forward** (which modules own what) → `COVERAGE.md`, then **6.2 reverse validation** (for every first-party file, ask "who owns me?" — orphans must be triaged into covered / excluded / unknown+RISK before completion). Then audit risks → finalize `RISKS.md` (aggregate from each MODULE.md `## Unknowns And Risks` + low-confidence cross-check results + orphan-derived unknowns).
