...
5. **Phase 2b** — full tree planning to leaf level. **Dispatch N parallel subagents** (one per top-level module from 2a), each exploring its own subtree and proposing child IDs. Assign every stable ID before writing any per-module doc content. MANIFEST `Module Tree` is complete after this phase. Parallelism is the default for non-trivial repos (see `analysis-protocol.md` "Subagent strategy" — concurrency budget, what each subagent can/cannot write).
...
8. **Phase 4** — fill module docs: **non-leaves first** (establish module-wide invariants), **then leaves** (reference parent invariants by ID). Use **batched writer**: group cohesive leaves into batches of 5–10 (same parent subtree, shared invariants, or same workflow stage); dispatch one writer subagent per batch; each batch's writer produces all leaf docs in one call. Multiple batches run in parallel (up to 6 concurrent). Every leaf doc is then passed through **Phase 4.4 Quality Gate** — 6 mechanical pass/fail checks (frontmatter completeness, code-map anchors, interactions ≥1+1, modification guide 3-bucket, leaf certificate ticked, END-OF-MODULE sentinel). Failures route back to writer for targeted patch (not full rewrite); 3-round escalation to `confidence: low` + R####. For high-risk leaves (auth / payment / consensus / crypto / persistence-migration / public-API / signing / governance), the orchestrator additionally does a read-back verification pass — re-grep cited anchors, spot-check inbound/outbound, verify invariants have Evidence Log references.
9. **Phase 5** — stitch global interaction maps under `interactions/`.
10. **Phase 6/7** — audit coverage in two passes: **6.1 forward** (which modules own what) → `COVERAGE.md`, then **6.2 reverse validation** (for every first-party file, ask "who owns me?" — orphans must be triaged into covered / excluded / unknown+RISK before completion). Then audit risks → finalize `RISKS.md` (aggregate from each MODULE.md `## Unknowns And Risks` + low-confidence module flags + orphan-derived unknowns + Phase 4.4 Gate persistent-failure escalations).
